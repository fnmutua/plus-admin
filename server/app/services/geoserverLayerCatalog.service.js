...
function layerMatchesSearch(layer, search) {
  if (!search) return true
  const needle = normalizeLayerName(search)
  const name = normalizeLayerName(layer.name)
  const title = normalizeLayerName(layer.title)
  return name === needle || title === needle || name.includes(needle) || title.includes(needle)
}
...
async function getPaginatedLayerCatalog({ page, limit, countyId, search, forceRefresh = false }) {
  const allLayers = await fetchLayerCatalogFromCapabilities(forceRefresh)

  let filtered = allLayers
  let optionsSource = allLayers
  let countyBbox = null

  if (countyId) {
    countyBbox = await getCountyBbox(countyId)
    if (countyBbox) {
      filtered = filtered.filter((layer) => layerBboxIntersectsCounty(layer.bbox, countyBbox))
      optionsSource = filtered
    }
  }

  if (search) {
    filtered = filtered.filter((layer) => layerMatchesSearch(layer, search))
    optionsSource = optionsSource.filter((layer) => layerMatchesSearch(layer, search))
  }
  ...
  return {
    data,
    total,
    options: toLayerOptions(optionsSource),
