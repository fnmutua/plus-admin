...
exports.xgenerateTimelinePDF = async (req, res) => {
  try {
    ...
    try {
      ...
      currentY -= 60; // Adjust for logo height and spacing
    } catch (e) {
  ...
  // Draw title
const titleText = `Grievance Timeline: ${grievance_id}`;
...
currentY -= lines.length * lineHeight + 20; // Adjust for wrapped lines and spacing
    ...
    for (let i = 0; i < sortedEvents.length; i++) {
      ...
      // Draw event marker (circle on timeline)
      page.drawCircle({
        x: timelineX,
        y: currentY + 5,
        size: 5,
      ...
      // Draw connector line from marker to event text
      page.drawLine({
        start: { x: timelineX + 10, y: currentY + 5 },
        end: { x: timelineX + 30, y: currentY + 5 },
        thickness: 1,
        color: rgb(0, 0, 0),
      });
      ...
      // Adjust currentY for the title
      currentY -= (titleLines.length - 1) * lineHeight;
    ...
    page.drawLine({
      start: { x: timelineX, y: timelineStartY },
      end: { x: timelineX, y: timelineEndY + 5 }, // Fit to last event
      thickness: 2,
      color: rgb(0, 0.2, 0.8),
    });
    ...
    // Save the PDF to the /data/grievances directory
    fs.writeFileSync(uploadPath, pdfBytes);
    ...
    res.status(200).send({
      code: '0000',
      message: 'Timeline PDF generated and saved successfully',
      filePath: uploadPath,
    });
  } catch (error) {
...
exports.generateTimelinePDF = async (req, res) => {
  try {
    ...
    res.status(200).send(Buffer.from(pdfBytes)); // 👈 Ensure this is a Buffer
