...
async function sendSMS(sms_obj) {
  // Send OTP via Leopard (not implemented in this code snippet)
  const url = "https://quicksms.advantasms.com/api/services/sendotp/";
  let encrytped_name= [Sequelize.fn('PGP_SYM_DECRYPT', Sequelize.cast(Sequelize.col('grievance.name'), 'bytea'),'***REDACTED***'),'name']
...
exports.getGrievances =async (req, res) => {
  ...
  var attributes = []
    for( let key in   db.models.grievance.rawAttributes ){
     ...
     let encrytped_national_id= [Sequelize.fn('PGP_SYM_DECRYPT', Sequelize.cast(Sequelize.col('grievance.national_id'), 'bytea'),'***REDACTED***'),'national_id']
...
exports.getGrievanceById = async (req, res) => {
      ...
      let decryptedName = [Sequelize.fn('PGP_SYM_DECRYPT', Sequelize.cast(Sequelize.col('grievance.name'), 'bytea'), '***REDACTED***'), 'name'];
      attributes.push(decryptedName);
    
      let decryptedNationalId = [Sequelize.fn('PGP_SYM_DECRYPT', Sequelize.cast(Sequelize.col('grievance.national_id'), 'bytea'), '***REDACTED***'), 'national_id'];
