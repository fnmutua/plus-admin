...
// Get imagery layers for a settlement (strict settlement_id linkage)
exports.getSettlementImageryLayers = async (req, res) => {
  try {
    ...
    const intersectingLayers = await imageryLayerService.getLayersForSettlement(settlementId)

    return res.status(200).json({
      message: 'Imagery layers retrieved successfully',
      code: '0000',
      data: intersectingLayers,
    })
  } catch (error) {
  ...
  console.log(log_object)
     ...
     // Legacy logs table writes are disabled; auditlogs is the source of truth.
      await logAudit({
        ...
        entityType: log_object.model || 'unknown',
        entityId: log_object.entity_id != null ? String(log_object.entity_id) : null,
        resource: log_object.resource || 'POST /api/v1/all/*',
...
exports.checkDocuments = async (req, res) => {
  ...
  try {
    ...
    for (const doc of documents) {
      ...
      results.push({
        name,
        document_id: existingDoc ? existingDoc.id : null,
        exists: !!existingDoc,
...
// ============================================
// OPTIMIZED MAP ENDPOINTS
// ============================================

/**
 * Get optimized settlements with pre-computed centroids and server-side filtering
 * Returns only point geometries (centroids) for better performance
 */
exports.getOptimizedSettlements = async (req, res) => {
  try {
    ...
    console.log(`[getOptimizedSettlements] Fetching ${model} with ${parsedFilters.length} filters`);
 ...
 */
exports.getBatchGeometries = async (req, res) => {
  try {
    const { model, ids, cache_key } = req.body;
    ...
    if (ids && Array.isArray(ids) && ids.length > 0) {
      const idList = ids.map(id => typeof id === 'string' ? `'${id.replace(/'/g, "''")}'` : id).join(', ');
      whereClause = `id IN (${idList}) AND ${whereClause}`;
    }
    ...
    console.log(`[getBatchGeometries] Fetching ${model}${ids ? ` with ${ids.length} IDs` : ' (all)'}`);
    ...
    console.error('Error in getBatchGeometries:', error);
    res.status(500).send({
      message: 'Internal server error',
      code: 'SERVER_ERROR',
      error: error.message
 ...
 */
exports.getCountiesList = async (req, res) => {
  try {
    const { 
      model = 'county',
      filters = [],
    ...
    console.error('Error in getCountiesList:', error);
    res.status(500).send({
      message: 'Internal server error',
      code: 'SERVER_ERROR',
      error: error.message
 ...
 */
exports.getSubcountiesList = async (req, res) => {
  try {
    const { 
      model = 'subcounty',
      county_id
    ...
    console.error('Error in getSubcountiesList:', error);
    res.status(500).send({
      message: 'Internal server error',
      code: 'SERVER_ERROR',
      error: error.message
