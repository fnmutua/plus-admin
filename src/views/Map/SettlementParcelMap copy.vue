...
const loadMap = async () => {
  ...
  nmap.value.on('load', async () => {
    ...
    console.log('nmap.value', nmap.value)
    nmap.value.addLayer({
      'id': 'Parcels',
      "type": "fill",
      'source': 'parcels',
  ...
  facilityData.value[prop].features[0].geometry.type === 'LineString' || 
  facilityData.value[prop].features[0].geometry.type === 'MultiLineString'
) {
  const layer = {
    id: prop,
    type: 'line',
...
.full-height {
  ...
  overflow: auto;
  /* Add scrollbars if content exceeds viewport height */
}
</style>