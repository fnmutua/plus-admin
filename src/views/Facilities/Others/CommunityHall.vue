...
const getFilteredData = async () => {
  ...
  const formData: any = {
    ...
    curUser: 1,
    model: facilityModel,
    searchField: 'community_hall_name',
...
const viewProfile = (row: any) => {
  push({ name: 'CommunityHallDetails', params: { id: row.id } })
}
...
const getName = (row: any) => row?.name || row?.community_hall_name || 'N/A'
const getType = (row: any) => row?.use || row?.type || 'N/A'
const getCondition = (row: any) => row?.condition || row?.Condition || 'N/A'
...
const openEditForm = (facilityData: any) => {
  ...
  editForm.name = facilityData.name || facilityData.community_hall_name || ''
  editForm.use = facilityData.use || ''
  editForm.usage_fee = facilityData.usage_fee ?? undefined
  editForm.condition = facilityData.condition || ''
  editForm.ownership = facilityData.ownership || ''
  editForm.owner = facilityData.owner || ''
    ...
    if (valid) {
      try {
        const formData: any = {
          model: facilityModel,
          name: editForm.name,
          community_hall_name: editForm.name,
          use: editForm.use,
...
/>
  ...
  >
    ...
    >
      <el-divider content-position="left">Hall Information</el-divider>

      <el-form-item label="Name" prop="name">
        <el-input v-model="editForm.name" placeholder="Enter hall name" />
      </el-form-item>

      <el-form-item label="Use">
        <el-input v-model="editForm.use" placeholder="Enter use" />
      </el-form-item>
      ...
      <el-form-item label="Owner">
        <el-input v-model="editForm.owner" placeholder="Enter owner" />
      </el-form-item>
    </el-form>
