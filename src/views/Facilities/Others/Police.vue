...
const createPermission = 'police_station:create'
...
const viewProfile = (row: any) => {
  push({ name: 'PoliceDetails', params: { id: row.id } })
}
...
const getName = (row: any) => row?.name || row?.PC_Name || 'N/A'
const getType = (row: any) => row?.PC_Type || row?.type || 'N/A'
const getCondition = (row: any) => row?.PC_Condition || row?.condition || row?.Condition || 'N/A'
