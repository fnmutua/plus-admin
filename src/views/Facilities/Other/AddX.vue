...
import { ElCard, ElCascader, ElCascaderPanel, ElDialog, ElMessage, ElUpload, ElTreeSelect } from 'element-plus'
  ...
  if (currentStep.value < totalSteps.value - 1 && dynamicFormRef) {
    ...
    formInstance.value.validate((valid: boolean) => {
      if (valid) {
        ...
        const nextFields = formFields[currentStep.value];
       console.log('current step',currentStep.value)
        currentStepFieldsFiltered.value = nextFields;
...
const getFieldChangeHandler = (fieldName: string) => {
  ...
  if (fieldName == 'type') {
    handleChangeType(formData[fieldName])
  }
...
const handleChangeType= async (value: any) => {
  console.log(' changed:', value);
  console.log(currentStepFields.value);
  currentStepFieldsFiltered.value = currentStepFields.value.filter(field => {
  //  return field.relatives == value;
  //  return field.relatives === value || field.relatives === 'all';
    return field.relatives.includes(value) || field.relatives.includes('all');

  });
  console.log('filtred', currentStepFieldsFiltered.value);
 // formKey.value += 1;
};
