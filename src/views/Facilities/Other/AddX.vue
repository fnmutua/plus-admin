...
v-for="(field, index) in currentStepFields" :key="index" :span="24" :xs="24" :sm="24" :md="12" :lg="8"
            :xl="8">
            <el-form-item :label="field.label" :prop="field.name" v-if="field.show === 'true'" >
...
const handleChange = (selected) => {
  ...
  if (selection === 'primary_substation' || selection === 'powerline' || selection === 'floodlight' || selection === 'secondary_substation') {
    const rating = formFields.flat().find((f) => f.name === 'rating');
    
        rating.show='true'
    
        const number_phases = formFields.flat().find((f) => f.name === 'number_phases');
        number_phases.show='true'
  ...
  if (selection === 'floodlight') {
    ...
    const height = formFields.flat().find((f) => f.name === 'height');
    height.show='true'

    const date_install = formFields.flat().find((f) => f.name === 'date_install');
    date_install.show='true'
  ...
  if (selection === 'toilet' || selection === 'shower') {
    ...
    console.log(selection)
     const stances = formFields.flat().find((f) => f.name === 'number_stances');
    stances.show = 'true'
  ...
  if (selection === 'illegal_dumping_site' || selection === 'legal_dumping_site' || selection === 'treatment_center' || selection === 'collection_point' || selection === 'waste_mgmt_project' || selection === 'other_waste_mgmt') {
     const cost_per_use = formFields.flat().find((f) => f.name === 'cost_per_use');
    cost_per_use.show = 'true'
    
    const type_waste = formFields.flat().find((f) => f.name === 'type_waste');
    type_waste.show='true'

  } else {   
    const cost_per_use = formFields.flat().find((f) => f.name === 'cost_per_use');
    cost_per_use.show = 'false'
    
    const type_waste = formFields.flat().find((f) => f.name === 'type_waste');
    type_waste.show='false'

  }

  if (selection === 'police_stn' || selection === 'police_post' || selection === 'chiefs_camp') {
 
    const number_staff = formFields.flat().find((f) => f.name === 'number_staff');
    number_staff.show='true'

    const number_vehicles = formFields.flat().find((f) => f.name === 'number_vehicles');
    number_vehicles.show='true'

      } else {
    const number_staff = formFields.flat().find((f) => f.name === 'number_staff');
    number_staff.show='false'

    const number_vehicles = formFields.flat().find((f) => f.name === 'number_vehicles');
    number_vehicles.show='false'
  }

  if (selection === 'other_hazard' || selection === 'landslide' || selection === 'flooding' || selection === 'fire') {
 
    const hazard = formFields.flat().find((f) => f.name === 'hazard');
    hazard.show='true'
