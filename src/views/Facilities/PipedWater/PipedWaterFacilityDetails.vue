...
const loadProfile = async () => {
  ...
  try {
    facilityLines.value = []
    const res = await getSettlementListByCounty({ limit: 1, page: 1, curUser: 1, model: 'piped_water', searchField: 'name', searchKeyword: '', filters: ['id'], filterValues: [[id]], associated_multiple_models: ['settlement', 'county', 'subcounty', 'ward'] })
    d.value = res.data[0] || {}
...
const loadMapData = async () => {
  ...
  try {
    const geoRes = await getOneGeo({ model: 'piped_water', id })
    ...
    if (settlementId) {
      ...
      if (sGeo?.features?.length) {
        ...
        if (mapRef.value?.map && window.google?.maps) {
          ...
          facilityLines.value.forEach(l => l.path.forEach((pt: any) => bounds.extend(pt)))
...
const loadDocuments = async () => {
  ...
  try {
    const res: any = await getLinkedDocuments({ entity_type: 'piped_water', entity_id: Number(id) })
...
const handleUnlinkDocument = async (row: any) => {
  try {
    await unlinkDocument({ document_id: row.id, entity_type: 'piped_water', entity_id: Number(id) })
    ...
    <el-card v-loading="loading">
      <template #header>
        <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
          ...
          <span style="font-size: 18px; font-weight: 600;">{{ d.name || 'Piped Water Scheme' }}</span>
          <el-tag v-if="d.condition" size="small" type="info">{{ d.condition }}</el-tag>
        ...
        <el-tab-pane label="Profile" name="profile">
          ...
          <div :class="prefixCls">
            <div :class="`${prefixCls}-header`" style="height:50px;display:flex;justify-content:space-between;align-items:center;padding:0 12px;cursor:pointer;border-bottom:1px solid var(--el-border-color-lighter);" @click="collapsed.details = !collapsed.details">
              <span style="font-weight:600;">Coverage &amp; Capacity</span><Icon :icon="collapsed.details ? 'ep:arrow-down' : 'ep:arrow-up'" />
            </div>
            <ElCollapseTransition><div v-show="!collapsed.details" style="padding: 12px;">
              <el-descriptions :column="2" border>
                ...
                <el-descriptions-item label="Number of Connections">{{ fmt(d.number_connections) }}</el-descriptions-item>
                <el-descriptions-item label="Persons Served">{{ fmt(d.number_persons_served) }}</el-descriptions-item>
              </el-descriptions>
            </div></ElCollapseTransition>
          </div>
