...
onMounted(async () => {
  // Check route query params first (from PipedWater.vue AddFacility function)
...
const steps = [
  ...
  { title: 'Draw Pipeline', description: 'Draw the pipeline on the map' },
  { title: 'Complete Details', description: 'Fill in pipeline information' }
]
...
const pipelinePolyline = ref<any>(null)
const settlementGeo = ref<any>(null)
const pipelineGeometry = ref<any>(null)
...
const handleSettlementChange = async (settlementId: any) => {
  ...
  try {
    ...
    if (res.data[0]?.json_build_object?.features) {
      ...
      if (settlement) {
        pipedWaterForm.settlement_id = settlementId
        pipedWaterForm.county_id = settlement.county_id || ''
        pipedWaterForm.subcounty_id = settlement.subcounty_id || ''
        pipedWaterForm.ward_id = settlement.ward_id || ''
      }
    ...
    window.google.maps.event.addListener(
      ...
      (event: any) => {
        if (event.type === window.google.maps.drawing.OverlayType.POLYLINE) {
          ...
          pipelinePolyline.value = polyline
          ...
          // Convert to MultiLineString for piped_water model
          const geometry = {
          ...
          pipelineGeometry.value = geometry
          pipedWaterForm.geom = geometry

          const length = turf.length({ type: 'LineString', coordinates }, { units: 'meters' })
          console.log('Pipeline length:', length, 'meters')
...
const submitForm = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      if (!pipedWaterForm.geom) {
        ElMessage.error('Please draw the pipeline on the map')
        return
      }
      ...
      try {
        ...
        const formData = {
          ...
          length: turf.length({ type: 'LineString', coordinates: pipedWaterForm.geom.coordinates[0] }, { units: 'meters' }),
          isApproved: 'Pending',
        ...
        if (res.code === '0000') {
          ElMessage.success('Piped water scheme created successfully')
          router.push({ name: 'PipedWater' })
        } else {
          ElMessage.error('Failed to create piped water scheme')
        }
      } catch (error) {
        console.error('Error creating piped water scheme:', error)
        ElMessage.error('Failed to create piped water scheme')
      }
    }
  })
...
const goBack = () => {
  if (currentStep.value > 0) {
    currentStep.value--
    if (currentStep.value === 0) {
      if (pipelinePolyline.value) {
        pipelinePolyline.value.setMap(null)
        pipelinePolyline.value = null
      }
...
<template>
  <div class="add-pipedwater-container">
    <el-card>
      <template #header>
        <div class="header-content">
          <el-button :icon="ArrowLeft" @click="goBack" text>Back</el-button>
          <h2>Add New Piped Water Scheme</h2>
        </div>
      </template>
    ...
    <el-drawer
      ...
      title="Piped Water Scheme Details"
