...
import { ElButton, ElSelect, MessageParamsWithType, UploadProps, ElOptionGroup, ElBadge, ElDescriptions, ElDescriptionsItem, ElOption, FormInstance } from 'element-plus'
...
const submitMoreDocuments = async () => {
  ...
  if (proceed) {
    ...
    for (var i = 0; i < morefileList.value.length; i++) {
      ...
      formData.append('field_id', 'water_point_id')

      formData.append('size', (morefileList.value[i].raw.size / 1024 / 1024).toFixed(2))
      formData.append('code', uuid.v4())
      formData.append('water_point_id', currentRow.value.id)
...
const Review = (data: TableSlotDefault) => {
  ...
  water_point_raw.value.name = data.row.name
  water_point_raw.value.reg_status = data.row.reg_status
  water_point_raw.value.ownership_type = data.row.ownership_type
  water_point_raw.value.owner = data.row.owner
  water_point_raw.value.user = data.row.user.name + ' | ' + data.row.user.email
  water_point_raw.value.date = data.row.createdAt
  ...
  ruleForm.capacity = data.row.capacity
  ruleForm.cost_20l_jerrican = data.row.cost_20l_jerrican
  ruleForm.owner = data.row.owner
  ruleForm.ownership_type = data.row.ownership_type
  ruleForm.depth = data.row.depth
...
layout="sizes, prev, pager, next, total" v-model:currentPage="currentPage"
    ...
    <el-dialog v-model="ShowReviewDialog" @close="handleClose" :title="formHeader" :width="reviewWindowWidth" draggable>
      <el-descriptions title="" direction="vertical" :column="2" size="small" border>
        <el-descriptions-item label="Name">{{ water_point_raw.name }}</el-descriptions-item>
        <el-descriptions-item label="Status" :span="2">{{ water_point_raw.reg_status }}</el-descriptions-item>
        <el-descriptions-item label="Type">{{ water_point_raw.ownership_type }}</el-descriptions-item>
